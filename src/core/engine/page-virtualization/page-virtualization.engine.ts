...
import ThumbnailViewer from '../../../viewer/ui/thumbnail-viewer.component';
import WebViewer from '../../../viewer/ui/web-viewer.component';
import AnnotationLayer from '../../../viewer/ui/annotation-layer.component';
import { AnnotationManager } from '../../../viewer/managers/annotation-manager.manager';
import { SelectionManager } from '../../../viewer/managers/selection.manager';
import SearchHighlighter from '../../../viewer/managers/search-highlighter.manager';
...
import { PageRenderQueue } from './page-render-queue.engine';
import { CachedPageInfo } from './types';
...
class PageVirtualization {
  ...
  private _renderQueue: PageRenderQueue;
  ...
  constructor(
    ...
    this._renderQueue = new PageRenderQueue(this._processQueuedPage.bind(this), this._shouldSkipQueuedPage.bind(this));
  ...
  private _shouldSkipQueuedPage(pageInfo: CachedPageInfo): boolean {
    const distance = Math.abs(pageInfo.pageNumber - this.state.currentPage);
    if (distance > this._aggressiveCancelDistance) {
      return true;
    }
    if (!pageInfo.isVisible) return true;
    if (pageInfo.isTransitioningToFullRender) return true;
    if (pageInfo.isFullyRendered && pageInfo.renderedScale === this.state.scale) {
      return true;
    }
  ...
  private async _processQueuedPage(pageInfo: CachedPageInfo): Promise<void> {
    this._aggressiveCancelRenders();
    ...
    Logger.info(`Processing render queue for page ${pageInfo.pageNumber}`, {
      queueLength: this._renderQueue.size,
    });
  ...
  private _queuePageForRender(pageInfo: CachedPageInfo, priority: number = 1): void {
    this._renderQueue.enqueue(pageInfo, priority);
  }
  ...
  private _removeFromRenderQueue(pageNumber: number): boolean {
    return this._renderQueue.remove(pageNumber);
  }
  ...
  private _emergencyCancelAll(): void {
    ...
    this._renderQueue.filter((item) => item.priority <= 1);
  ...
  private _cancelOffscreenRenders(): void {
    ...
    this._renderQueue.filter((item) => item.pageInfo.isVisible);
