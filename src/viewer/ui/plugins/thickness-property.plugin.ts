...
/**
 * Configuration for thickness property plugin
 */
export interface ThicknessPropertyPluginConfig extends BasePropertyPluginConfig {
  /** Minimum thickness value (default: 1) */
  min?: number;
  /** Maximum thickness value (default: 10) */
  max?: number;
  /** Step value for slider (default: 0.1) */
  step?: number;
  /** Custom display format function (default: shows as "X pt") */
  displayFormat?: (value: number) => string;
}

/**
 * Plugin for stroke width/thickness control with slider
 */
export class ThicknessPropertyPlugin extends BasePropertyPlugin {
  private thicknessConfig: ThicknessPropertyPluginConfig;
  ...
  constructor(config: ThicknessPropertyPluginConfig = {}) {
    super('thickness-property', config);
    this.thicknessConfig = {
      min: config.min ?? 1,
      max: config.max ?? 10,
      step: 0.1,
      displayFormat: config.displayFormat ?? ((v) => `${Math.round(v)} pt`),
    };
  }

  protected renderPropertyControl(container: HTMLElement, context: AnnotationContext): void {
    const label = this.config.label || 'Thickness';
    const { min, max, displayFormat } = this.thicknessConfig;

    container.classList.add(PDF_VIEWER_CLASSNAMES.A_ANNOTATION_SHAPE_PROPERTIES_THICKNESS);
    ...
    this.currentValue = context.stateManager.state.drawConfig.strokeWidth;
  ...
  private handleSliderMove(
    ...
    this.button!.textContent = displayFormat(this.currentValue);
    this.valueLabel!.textContent = displayFormat(this.currentValue);
    context.stateManager.updateDrawConfig({ strokeWidth: this.currentValue });
  }
  ...
  private toggleDropdown(): void {
    ...
    if (isVisible) {
      ...
      setTimeout(() => {
        this.updateThumb(this.currentValue, this.thicknessConfig.min!, this.thicknessConfig.max!);
      }, 0);
    }
  }
  ...
  protected onDrawConfigChange(drawConfig: any): void {
    if (drawConfig.strokeWidth !== undefined && drawConfig.strokeWidth !== this.currentValue) {
      this.currentValue = drawConfig.strokeWidth;
      if (this.button && this.thicknessConfig.displayFormat) {
        this.button.textContent = this.thicknessConfig.displayFormat(this.currentValue);
      }
      this.updateThumb(this.currentValue, this.thicknessConfig.min!, this.thicknessConfig.max!);
    }
  }
